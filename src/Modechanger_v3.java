...
public class Modechanger_v3 {
 
	static String controlfile; 
	static String filenamein, filenameout, modefile;
	int linecount=0;
	String [][] libConvert;
	...
	public static void main(String[] args) throws Exception {
        controlfile = args[0];
		Modechanger_v3 app = new Modechanger_v3();
        app.readCtlFile();
		app.sizeLib();
		app.buildLib();
	 ...
	 public void readCtlFile() throws IOException {
		...
		while ( (line0 = br0.readLine()) != null ){
			...
			while (st.hasMoreTokens()){ 			// make sure there is stuff to get
				temp = st.nextToken();
				if(temp.equals("Modechanger_InputFile")){
					filenamein = st.nextToken();
				}
				if(temp.equals("Modechanger_OutputFile")){
					filenameout = st.nextToken();
				}
				if(temp.equals("ModeFile")){
					modefile = st.nextToken();
				}
			}
		}
	 }
		...
		public void sizeLib() throws IOException {
			...
			BufferedReader br1 = new BufferedReader(new FileReader(new File(modefile)));
			String line1 = ""; 							
	  ...
	  public void buildLib() throws IOException {
		...
		BufferedReader br2 = new BufferedReader(new FileReader(new File(modefile)));
		int tokenCount;
		...
		while ( (line2 = br2.readLine()) != null ){
			...
			while (st2.hasMoreTokens()){ 			// make sure there is stuff to get
				if(j<libConvert.length){
					...
					libConvert[j][0] = "MODE\\="+st2.nextToken();
					//System.out.println(libConvert[j][0]);
					libConvert[j][1] = "MODEE\\="+st2.nextToken();
